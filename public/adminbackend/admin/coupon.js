...
$('.get-quick-view').on('click', function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            // $('#quick-view-modal').empty().html(data.coupon);
            // console.log();
            $('#coupon_title').text(data.coupon_title);
            $('#coupon_code').text(data.coupon_code);
            $('#min_purchase').text(data.minimum_purchase);
            $('#max_discount').text(data.coupon_max_discount);
            $('#coupon_start').text(data.coupon_start);
            $('#coupon_end').text(data.coupon_end);
            $('#discount_for').text(data.coupon_for == 0 ? 'All Customers' : 'Loyalty Customers');
            $('#coupon_discount').text(data.coupon_discount);
            $('#symbol').text(data.discount_type == 'percentage' ? '৳' : '%');
        }
    });
});
